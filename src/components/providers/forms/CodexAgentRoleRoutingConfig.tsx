...
export function CodexAgentRoleRoutingConfig({
  ...
  const handleFetchModels = async () => {
    const config = selectedProvider?.settingsConfig?.config;
    const baseUrl = frontend.providerId
      ? extractCodexBaseUrl(typeof config === "string" ? config : "") || ""
      : ownerBaseUrl;
    const apiKey = frontend.providerId
      ? providerApiKey(selectedProvider)
      : ownerApiKey;
    const isFullUrl = frontend.providerId
      ? selectedProvider?.meta?.isFullUrl === true
      : ownerIsFullUrl;
    const customUserAgent = frontend.providerId
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder={t("providerAdvanced.agentRoleFollowMainAgent", {
                    defaultValue: "Follow main agent",
                  })}
                />
                ...
                {capabilityModelUnknown && (
                  <p className="text-sm text-amber-600" role="alert">
                    {t("providerAdvanced.agentRoleUnknownCapabilityModel", {
                      defaultValue:
                        "This model is not present in the visible shared catalog. Codex will use fallback model metadata.",
                    })}
                  </p>
                )}
              </div>
