...
mod tests {
    ...
    #[test]
    fn response_body_too_large_bypasses_all_retry_matchers() {
        let provider = provider(LocalProxyRetryPolicy {
            enabled: Some(true),
            max_retries: 0,
            retry_delay_ms: 1,
            custom_messages: vec!["上游响应体超过大小上限".to_string()],
            error_types: vec![
        ...
        assert!(policy
            .match_error(&ProxyError::ResponseBodyTooLarge(128 * 1024 * 1024 + 1))
            .is_none());
    }
