...
pub fn write_codex_live_atomic(
    ...
    let config = config_text_opt.unwrap_or_default();
    validate_config_toml(config)?;
    crate::services::config_guard::commit_files(&crate::app_config::AppType::Codex, &[
        (get_codex_auth_path(), serde_json::to_vec_pretty(auth).map_err(|source| AppError::JsonSerialize { source })?),
        (get_codex_config_path(), config.as_bytes().to_vec()),
    ])
}
...
pub fn prepare_codex_config_text_with_model_catalog(
    ...
    let (text, catalog) = plan_codex_config_text_with_model_catalog(settings, config_text, profile)?;
    if let Some(catalog) = catalog {
        crate::services::config_guard::preflight_files(&crate::app_config::AppType::Codex, &[
            (get_codex_config_path(), text.as_bytes().to_vec()),
            (get_codex_model_catalog_path(), serde_json::to_vec_pretty(&catalog).map_err(|source| AppError::JsonSerialize { source })?),
        ])?;
...
fn plan_codex_config_text_with_model_catalog(
    settings: &Value, config_text: &str, profile: CodexCatalogToolProfile,
    ...
    if let Some(catalog) = codex_model_catalog_from_settings(settings, config_text, profile)? {
        ...
        Ok((set_codex_native_web_search_field(&config_text, disable_web_search)?, None))
    }
}
...
pub fn write_codex_provider_live_with_catalog(
    ...
    let _permission = crate::app_management::permit_path(&get_codex_config_dir())?;
    let files = codex_provider_files_with_catalog(settings, category, auth, config_text, profile)?;
    crate::services::config_guard::commit_file_changes(&crate::app_config::AppType::Codex, &files)
}

fn codex_provider_files_with_catalog(settings: &Value, category: Option<&str>, auth: &Value,
    config_text: Option<&str>, profile: CodexCatalogToolProfile) -> Result<Vec<(PathBuf, Option<Vec<u8>>)>, AppError> {
    let (text, catalog) = match config_text {
        Some(text) => { let (text, catalog) = plan_codex_config_text_with_model_catalog(settings, text, profile)?; (Some(text), catalog) },
        None => (None, None),
    };
    let plan = plan_codex_live_write(category, auth, text.as_deref(), crate::settings::preserve_codex_official_auth_on_switch())?;
    let mut files = vec![(get_codex_config_path(), Some(plan.config_text.unwrap_or_default().into_bytes()))];
    if plan.write_full_auth { files.push((get_codex_auth_path(), Some(serde_json::to_vec_pretty(auth).map_err(|source| AppError::JsonSerialize { source })?))); }
    else if plan.remove_auth_file { files.push((get_codex_auth_path(), None)); }
    if let Some(catalog) = catalog { files.push((get_codex_model_catalog_path(), Some(serde_json::to_vec_pretty(&catalog).map_err(|source| AppError::JsonSerialize { source })?))); }
    Ok(files)
}

pub(crate) fn preflight_codex_provider_with_catalog(settings: &Value, category: Option<&str>, auth: &Value,
    config_text: Option<&str>, profile: CodexCatalogToolProfile) -> Result<(), AppError> {
    let files = codex_provider_files_with_catalog(settings, category, auth, config_text, profile)?;
    crate::services::config_guard::preflight_file_changes(&crate::app_config::AppType::Codex, &files)
}
...
pub fn write_codex_live_for_provider(
    ...
    if plan.write_full_auth {
        files.push((get_codex_auth_path(), Some(serde_json::to_vec_pretty(auth).map_err(|source| AppError::JsonSerialize { source })?)));
    } else if plan.remove_auth_file {
        files.push((get_codex_auth_path(), None));
    }
    crate::services::config_guard::commit_file_changes(&crate::app_config::AppType::Codex, &files)
}
...
"#;
    ...
    fn managed_chatgpt_login_matches_local_marker_and_workspace() {
        ...
        std::fs::write(
            get_codex_auth_path(),
            serde_json::to_vec_pretty(&other_user).expect("serialize other user's native login"),
        )
    ...
    fn legacy_managed_marker_migrates_by_user_without_breaking_refresh_rollback() {
        ...
        std::fs::write(
            get_codex_auth_path(),
            serde_json::to_vec_pretty(&auth_r1).expect("serialize rotated live auth"),
        )
