...
#[cfg(test)]
pub(crate) fn set_rollback_file_guard_pause_for_test(
    entered: Arc<tokio::sync::Notify>,
    release: Arc<tokio::sync::Notify>,
) {
...
pub(crate) async fn run_with_codex_provider_lifecycle_lock<T, F>(
    state: Arc<AppState>,
    label: &'static str,
    operation: F,
) -> Result<T, AppError>
...
pub(crate) async fn switch_codex_provider_for_failover(
    state: Arc<AppState>,
    provider_id: String,
) -> Result<bool, AppError> {
    run_codex_provider_mutation(state, "switch Codex Provider for failover", move |state| {
        let previous_provider_id = ProviderService::current(state.as_ref(), AppType::Codex)?;
        ProviderService::switch_under_proxy_transaction(
            state.as_ref(),
            AppType::Codex,
            &provider_id,
        )?;
        Ok(previous_provider_id != provider_id)
    })
    .await
}
...
mod tests {
    use super::{
        ...
        set_rollback_file_guard_pause_for_test, switch_codex_provider_for_failover,
    };
    use crate::app_config::AppType;
    ...
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    #[serial]
    async fn failover_switch_commits_provider_and_role_projection_together() {
        ...
        let switched =
            switch_codex_provider_for_failover(Arc::clone(&state), provider_b.id.clone())
                .await
                .expect("transactional failover succeeds");
    ...
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    #[serial]
    async fn failover_role_conflict_rolls_back_provider_proxy_and_role_state() {
        let home = TempHome::new();
        crate::settings::reload_settings().expect("reload isolated settings");
        let db = Arc::new(Database::memory().expect("init db"));
        configure_dynamic_proxy(&db).await;
        ...
        fs::write(&frontend_role, b"name = \"user-owned\"\n").expect("write user-owned role");

        let error = switch_codex_provider_for_failover(Arc::clone(&state), provider_b.id.clone())
            .await
            .expect_err("role conflict aborts failover transaction");
