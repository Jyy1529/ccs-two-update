...
";
    ...
    fn deleting_enabled_codex_role_owner_is_blocked() {
        with_test_home(|state, _| {
            ...
            assert_eq!(error.to_string(), "codex_agent_role_owner_delete_blocked:");
        });
    }
...
impl ProviderService {
    ...
    pub(crate) fn switch_under_proxy_transaction(
        ...
        // The caller holds the proxy transaction lock. Serialize against other
        // mutations for this app before deciding from the locked current state.
        let _switch_guard = if matches!(
            app_type,
            AppType::Claude | AppType::Codex | AppType::Gemini | AppType::GrokBuild
        ) {
