...
fn portable_fields_from_provider(
    ...
    PortableProviderFields {
        ...
        api_key,
        base_url,
        model: first_provider_model(app_type, &provider.settings_config),
    }
}
...
fn first_model_from_array(value: Option<&serde_json::Value>) -> Option<String> {
    value
        .and_then(serde_json::Value::as_array)
        .into_iter()
        .flatten()
        .find_map(|model| {
            nonempty_string(model.get("id")).or_else(|| nonempty_string(model.get("model")))
        })
}

fn first_model_from_object(value: Option<&serde_json::Value>) -> Option<String> {
    value
...
fn first_model_from_toml_table(value: &toml::Value) -> Option<String> {
    value
        .as_table()
        .into_iter()
        .flat_map(|models| models.keys())
...
fn first_provider_model(app_type: &AppType, settings: &serde_json::Value) -> Option<String> {
    match app_type {
        AppType::Claude | AppType::ClaudeDesktop => nonempty_string(
            settings
                .pointer("/env/ANTHROPIC_MODEL")
                .or_else(|| settings.pointer("/env/ANTHROPIC_DEFAULT_SONNET_MODEL")),
        ),
        AppType::Codex => nonempty_string(settings.get("model")).or_else(|| {
            settings
                .get("config")
                .and_then(serde_json::Value::as_str)
                .and_then(|config| {
                    config.parse::<toml::Value>().ok().and_then(|value| {
                        value
                            .get("model")
        ...
        AppType::Gemini => nonempty_string(settings.pointer("/env/GEMINI_MODEL")),
        AppType::GrokBuild => settings
            ...
            .and_then(|value| {
                value
                    ...
                    .filter(|value| !value.is_empty())
                    .map(str::to_string)
                    .or_else(|| value.get("model").and_then(first_model_from_toml_table))
            }),
        AppType::OpenCode => nonempty_string(settings.get("model"))
            .or_else(|| first_model_from_object(settings.get("models"))),
        AppType::OpenClaw | AppType::DeepSeek | AppType::Pi => {
            nonempty_string(settings.get("model"))
                .or_else(|| first_model_from_array(settings.get("models")))
        }
        AppType::Hermes => nonempty_string(settings.get("model"))
            .or_else(|| first_model_from_array(settings.get("models")))
            .or_else(|| first_model_from_object(settings.get("models"))),
    }
}
...
pub(crate) fn build_provider_for_target(
    ...
    if *target == AppType::Pi {
        let model = fields
            ...
            .ok_or_else(|| {
                AppError::localized(
                    "provider.transfer.pi.model_required",
                    "导入到 Pi 需要至少一个 model",
                    "Importing to Pi requires at least one model",
                )
            })?;
        return Ok(Provider::with_id(
            String::new(),
            fields.name.clone(),
            serde_json::json!({
                "name": fields.name,
        ...
        for requested_app in request.target_apps {
            ...
            let result = (|| -> Result<(String, String), AppError> {
                ...
                if target == AppType::Pi {
                    ...
                    ProviderService::add(state, target, provider, true)?;
                    return Ok((provider_id, import_name));
                }
