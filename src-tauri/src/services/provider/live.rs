...
pub(crate) fn preflight_codex_live_write_for_state(
    ...
    crate::codex_config::preflight_codex_provider_with_catalog(
        &effective.settings_config, effective.category.as_deref(), auth, config_str,
        crate::proxy::providers::resolve_codex_catalog_tool_profile(&effective),
    )
}
...
pub fn sync_current_to_live(state: &AppState) -> Result<(), AppError> {
    ...
    for app_type in AppType::all() {
        if !crate::app_management::is_managed(&app_type) { log::info!("Skipped provider sync for {}: unmanaged or pending", app_type.as_str()); continue; }
    ...
    // Skill sync
    for app_type in AppType::all() {
        if !crate::app_management::is_managed(&app_type) { continue; }
...
pub(crate) fn write_gemini_live(provider: &Provider) -> Result<(), AppError> {
    use crate::gemini_config::{
        ...
        get_gemini_env_path, serialize_env_file,
    };
    let _permission = crate::app_management::permit_path(&crate::gemini_config::get_gemini_dir())?;
    ...
    let mut config = config_to_write.unwrap_or_else(|| json!({}));
    let invalid = || AppError::Config("Gemini security/auth must be objects; no connection files were changed".into());
    let root = config.as_object_mut().ok_or_else(invalid)?;
    let security = root.entry("security").or_insert_with(|| json!({})).as_object_mut().ok_or_else(invalid)?;
    let auth = security.entry("auth").or_insert_with(|| json!({})).as_object_mut().ok_or_else(invalid)?;
    auth.insert("selectedType".into(), json!(selected_type));
    crate::services::config_guard::commit_files(&AppType::Gemini, &[
        (get_gemini_env_path(), serialize_env_file(&env_map).into_bytes()),
        (settings_path, serde_json::to_vec_pretty(&config).map_err(|source| AppError::JsonSerialize { source })?),
    ])
}
