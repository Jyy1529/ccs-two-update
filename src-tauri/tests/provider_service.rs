...
"#
    ...
    let before_config = std::fs::read(cc_switch_lib::get_codex_config_path()).unwrap();
    let error = ProviderService::switch(&state, AppType::Codex, "third-party")
        .expect_err("deleting user-owned OAuth login requires explicit review");
    assert!(matches!(error, cc_switch_lib::AppError::Config(_)));
    assert_eq!(std::fs::read(cc_switch_lib::get_codex_config_path()).unwrap(), before_config);
    assert_eq!(read_json_file::<serde_json::Value>(&cc_switch_lib::get_codex_auth_path()).unwrap(), live_auth);
    assert_eq!(ProviderService::current(&state, AppType::Codex).unwrap(), "legacy-provider");
    support::approve_configuration_conflicts(AppType::Codex, Some(&state));
    assert_eq!(ProviderService::current(&state, AppType::Codex).unwrap(), "third-party");
...
fn provider_service_switch_codex_official_accounts_write_auth_json() {
    ...
    let error = ProviderService::switch(&state, AppType::Codex, "official-b")
        .expect_err("replacing user-owned OAuth credentials requires review");
    assert!(matches!(error, cc_switch_lib::AppError::Config(_)));
    assert_eq!(
        read_json_file::<serde_json::Value>(&cc_switch_lib::get_codex_auth_path())
            .expect("read auth before confirmation"),
        live_auth_a
    );
    assert_eq!(
        ProviderService::current(&state, AppType::Codex).expect("current provider before review"),
        "official-a"
    );
    support::approve_configuration_conflicts(AppType::Codex, Some(&state));
    assert_eq!(
        ProviderService::current(&state, AppType::Codex).expect("current provider after review"),
        "official-b"
    );
...
fn provider_service_switch_claude_updates_live_and_state() {
    ...
    assert!(ProviderService::switch(&state, AppType::Claude, "new-provider").is_err(), "legacy live changes must be reviewed");
...
fn switch_claude_syncs_new_shared_keys_from_live_into_common_config() {
    ...
    let error = ProviderService::switch(&state, AppType::Claude, "b")
        .expect_err("foreign provider credentials require review before switching");
    assert!(matches!(error, cc_switch_lib::AppError::Config(_)));
    assert_eq!(read_json_file::<serde_json::Value>(&settings_path).unwrap(), live);
    assert_eq!(ProviderService::current(&state, AppType::Claude).unwrap(), "a");
    support::approve_configuration_conflicts(AppType::Claude, Some(&state));
    assert_eq!(ProviderService::current(&state, AppType::Claude).unwrap(), "b");
...
"#;
    ...
    let error = ProviderService::switch(&state, AppType::Codex, "b")
        .expect_err("removing the previous provider's legacy top-level token requires review");
    assert!(matches!(error, cc_switch_lib::AppError::Config(_)));
    assert_eq!(std::fs::read(cc_switch_lib::get_codex_config_path()).unwrap(), before_config);
    assert_eq!(ProviderService::current(&state, AppType::Codex).unwrap(), "a");
    support::approve_configuration_conflicts(AppType::Codex, Some(&state));
    assert_eq!(ProviderService::current(&state, AppType::Codex).unwrap(), "b");
...
fn recover_from_crash_without_backup_cleans_placeholder_instead_of_writing_it_back() {
    ...
    let error = futures::executor::block_on(state.proxy_service.recover_from_crash())
        .expect_err("unbaselined crash recovery requires review");
    assert!(error.contains("Configuration"));
    assert_eq!(
        read_json_file::<serde_json::Value>(&settings_path).expect("read unconfirmed recovery"),
        taken_over_live
    );
    support::approve_configuration_conflicts(AppType::Claude, Some(&state));
    futures::executor::block_on(state.proxy_service.recover_from_crash())
        .expect("finish confirmed crash recovery");
